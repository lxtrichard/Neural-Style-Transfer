...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        self.genA2B = ResnetGenerator(in_channels=3, out_channels=3, ngf=self.ch, n_blocks=self.n_res, img_size=self.img_size, light=self.light).to(self.device)
        self.genB2A = ResnetGenerator(in_channels=3, out_channels=3, ngf=self.ch, n_blocks=self.n_res, img_size=self.img_size, light=self.light).to(self.device)
        self.disGA = Discriminator(in_channels=3, ndf=self.ch, n_layers=7).to(self.device)
        self.disGB = Discriminator(in_channels=3, ndf=self.ch, n_layers=7).to(self.device)
        self.disLA = Discriminator(in_channels=3, ndf=self.ch, n_layers=5).to(self.device)
        self.disLB = Discriminator(in_channels=3, ndf=self.ch, n_layers=5).to(self.device)
    ...
    def train(self):
        ...
        plt.savefig('./results/loss/G_loss.png')
        ...
        plt.savefig('./results/loss/D_loss.png')
    ...
    def test(self):
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            A2B_s = np.concatenate((RGB2BGR(tensor2numpy(denorm(real_A[0]))), 
                                    RGB2BGR(tensor2numpy(denorm(fake_A2B[0])))),1)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A2B_%d.png' % (n + 1)), A2B * 255.0)
            cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'output_%d.png' % (n + 1)), A2B_s * 255.0)
